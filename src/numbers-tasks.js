...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const distanceResult = Math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2);
...
function getSumOfDigits(num) {
  ...
  const newStrotoArr = toStr.split('');
  function sumNum(x) {
    sum += Number(x);
  }
  newStrotoArr.flatMap(sumNum);
  return sum;
}
...
function toFixed(number, fractionDigits) {
  ...
  return numStr.slice(0, numStr.indexOf('.') + fractionDigits + 1);
}
...
function toPrecision(number, precision) {
  ...
  let resultStr = '';
  if (newStr.length < precision && Number.isInteger(number)) {
    resultStr += `${newStr}.`;
...
function getIntegerOnString(str, base) {
  if (typeof Number.parseInt(str, base) === 'number') {
